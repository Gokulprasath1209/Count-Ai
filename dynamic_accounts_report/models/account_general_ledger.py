...
class AccountGeneralLedger(models.TransientModel):
    ...
    def view_report(self, option, tag):
        ...
        move_line_ids = self.env['account.move.line'].search(
            [('parent_state', '=', 'posted')],order='date asc')
        ...
        for account in account_ids:
            ...
            for move_line in move_line_id:
                line_bal = move_line.debit - move_line.credit
                balance += line_bal
                move_line_data = [{
                    'id':move_line.id,
                    'date':move_line.date,
                    ...
                    'analytic_line_ids':move_line.analytic_line_ids,
                    'balance':balance
                }]
    ...
    def get_filter_values(self, journal_id, date_range, options, analytic,
        ...
        if analytic:
            analytic_line = self.env['account.account'].search([('id','in',analytic)])
            # analytic_line = self.env['account.analytic.line'].search(
            #     [('account_id', 'in', analytic)]).mapped('id')
            domain += [('account_id', 'in', analytic_line.ids)]
        if date_range:
            if date_range == 'month':
        ...
        whole_move_line_ids = self.env['account.move.line'].search(
            [('parent_state', '=', 'posted')], order='date asc')
        ...
        for account in account_ids:
            open_balance = 0
            add_open_balance = 0
            move_lines = whole_move_line_ids.filtered(
                lambda x: x.account_id == account)
            for i in move_lines:
                if date:
                    if i.date < start_date:
                        line_bal = i.debit - i.credit
                        open_balance += line_bal
                        add_open_balance += line_bal
            move_line_id = move_line_ids.filtered(
                lambda x: x.account_id == account)
            move_line_list = []
            for move_line in move_line_id:
            ...
            account_totals[account.display_name] = {
                ...
                'total_credit': round(sum(move_line_id.mapped('credit')), 2),
                'currency_id': currency_id,
                'opening': open_balance if date else 0.0,
                'account_id': account.id}
            account_dict['account_totals'] = account_totals
        return account_dict
            ...
            if report_action == 'dynamic_accounts_report.action_general_ledger':
                ...
                for account in data['account']:
                    ...
                    for rec in data['data'][account]:
                        ...
                        sheet.merge_range(row, col + 11, row, col + 12, rec[0]['balance'],
                                          txt_name)
                row += 1
