...
import { Component, onWillStart, useState, onMounted, useEffect,useRef,onWillUnmount } from "@odoo/owl";
...
export class KSDashboardNinjaOverview extends Component{
    setup(){
        ...
        onWillUnmount(()=>{
          ...
          let ks_dark =  localStorage.getItem('ks_current_color_mode')
          if (this.ks_dark == ks_dark && ks_dark == "ks-dark"){
            document.querySelector('html').setAttribute("data-color-mode","ks-dark")
          }
          document.getElementById("ks_darkmode_button")?.parentElement.classList.remove("d-none")
        })
        ...
        onMounted(()=>{
            ...
            document.querySelector(".ks-zoom-view")?.classList.add("d-none")
             document.getElementById("ks_darkmode_button")?.parentElement.classList.add("d-none")
             this.ks_dark = document.querySelector('html').getAttribute("data-color-mode")
             if (this.ks_dark === "ks-dark"){
                document.querySelector('html').setAttribute("data-color-mode","ks-light")
             }
