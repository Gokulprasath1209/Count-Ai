...
class KsDashboardNinjaBoard(models.Model):
    ...
    def write(self, vals):
        ...
        for rec in self:
            if 'ks_dashboard_menu_name' in vals:
                if self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board', raise_if_not_found=False) and self.env.ref(
            ...
            if 'ks_dashboard_group_access' in vals:
                if (self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board', raise_if_not_found=False) and
                        self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board').id == rec.id):
                    if self.env.ref('ks_dashboard_ninja.board_menu_root', False):
    ...
    def update_group_access_of_menus(self):
        all_dashboards = self.env['ks_dashboard_ninja.board'].search([])
        admin_group_id = self.env.ref('base.group_system').id
        for dashboard in all_dashboards:
            current_groups = dashboard['ks_dashboard_group_access']
            current_groups.ids.append(admin_group_id)
    ...
    def fetch_dashboard_overview(self, **kwargs):
        ...
        dashboards_info = {}
        list_view_count = ks_board_item_model.search_count(
            [('ks_dashboard_item_type', '=', 'ks_list_view'), ('ks_dashboard_ninja_board_id', '!=', False)])
